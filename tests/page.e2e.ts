...
test('selected subjects persist, reject stale values, and clear with focus restored', async ({
	...
	await expect(page.getByRole('heading', { name: 'Schedule heatmap' })).toBeHidden();
	await selectFirstSubject(page);
	await expect(page.getByRole('heading', { name: 'Schedule heatmap' })).toBeVisible();
	await expect(
		page.getByRole('img', { name: 'Relative enrollment: lower to higher.' })
	).toBeVisible();
...
test('five and seven days fit at 390px with the expected typography', async ({ page }) => {
	...
	await expect(page.getByRole('heading', { name: 'Schedule heatmap' })).toHaveCSS(
		'font-family',
		/Rubik Variable/
	);
	...
	let widths = await pageWidths(page);
